...
'''
################################ Project Overview #################################################################
Data science can be as much an art as .. well, a science. Understanding how different analyses work, what they 
can do and what their limitations are allows you to use them effectively. Choosing the right types of analysis 
can be quite tricky. When faced with a sea of data, how are you to condition and measure it in order to wring 
out some useful observations? Well, that is not the subject of this course. You will however get a taste of how 
to use Python in order to perform data analysis.

The analyses necessary to generate some interesting observations on GDP, energy use and population mechanics is 
below, however it needs some critical parts filled in. First you'll fill in some functions that will provide fundamental 
numerical measurements that will power the statistical analyses that come later. Afterward, you'll use the imported data 
and some of the functions you've written to generate some the observations. The sourcing and cleaning of the data, the 
analysis and conditioning and plotting has already been done for you!
...
Python is often used in data science and analysis because it is fairly easily to represent data as streams of 
values that can have actions applied to them. Many types of statistical analysis take this form; apply an operation 
to a stream of values or to corresponding values in different streams, then use the output of one result as the input 
to another. Making this linking of operations on streams of data simple to code and graph means Python is fairly 
well suited for investigating decently-sized sets of data.  
...
Once you fill in the computations to generate the necessary statistics, the following graphs will automatically 
be generated to demonstrate an observation/answer to each question:
...
class data_analysis:
    ...
    def normalize_series(self,data_series):
        ...
        To normalize the data_series list, 
        	->>>> each value in the data_series needs to be divided by the sum of all values in the list.
        ...
        #### FILL IN CODE HERE ####
        num_needed = ft.reduce(lambda a,b: (a+b), data_series)		
        normalized_series = list(map(lambda a: a/num_needed, data_series))	#returns a map object, just use list() to convert
    ...
    def compute_average_across_series(self,list_of_series):
		...
		For example, the first value in the output list will be the average of all the first values of all the sub-lists and
				the second value in the output list will be the average of all the second values of all the sub-lists, etc.
        ...
        Example:
            Input: [[0,10,20],[10,20,30]] 
                -> 
            Output: [5.0, 15.0, 25.0] (i think it should be [2, 15.0, 25.0])
            
        '''

        #### FILL IN CODE HERE ####
        # sum up all corresponding elements of series
        num_of_items = len(list_of_series)
        sum_list = list(map(sum, zip(*list_of_series)))#this was my attempt to not use a loop, I'm not sure if we were allowed to use zip() or not
        average_list = list(map(lambda a: (a/num_of_items), sum_list))
        return average_list
    ...
    def compute_standard_deviation_across_series(self,list_of_series,average_series):
        ...
        sum_list = []
        num_of_items = len(list_of_series)
        size_of_sublists = len(list_of_series[0])
        for sub in list_of_series:
            for i in range(size_of_sublists):
                if len(sum_list) < size_of_sublists:
                    sum_list.append(self.apply_power(sub[i]-average_series[i],2))
                else:
                    sum_list[i]+= self.apply_power(sub[i]-average_series[i],2)
        std_list = list( map(lambda a: (a**(1/2)),list(map(lambda b: (b/num_of_items), sum_list)) ))
        return std_list
    ...
    def compute_correlations(self,data_series_1,data_series_2):
        ...
        av1 = ft.reduce(lambda a,b: (a+b), data_series_1)/len(data_series_1)
        av2 = ft.reduce(lambda a,b: (a+b), data_series_2)/len(data_series_2)
        series_1 = list(map(lambda a: (a-av1), data_series_1))
        series_2 = list(map(lambda a: (a-av2), data_series_2))
        correlation_list = list(map(lambda a,b: (a*b), series_1, series_2))
    ...
    def compute_net_correlation(self,data_series_1,data_series_2):
        ...
        series_2 = list(map(lambda a: (a-av2), data_series_2))
        correlation_list = list(map(lambda a,b: (a*b), series_1, series_2 ))
        net_correlation = ft.reduce(lambda a,b: (a+b), correlation_list)
    ...
    def compute_ratio(self,data_series_1,data_series_2):
        ...
        #### FILL IN CODE HERE ####
        if 0 in data_series_2:
            print("There is a zero in data_series_2: error @ compute_ratio")
            exit()
        ratio_list = list(map(lambda a,b: (a/b), data_series_1, data_series_2))
    ...
    def compute_average_of_series(self,data_series):
        ...
        series_average = (ft.reduce(lambda b,c: (b+c), data_series))/len(data_series)
    ...
    def compute_standard_deviation_of_series(self,data_series,average_of_series):
        ...
        '''
        #### FILL IN CODE HERE #### 
        num_of_items = len(data_series)
        sum_list = list(map( lambda a: (a**2), list(map(lambda a:(a - average_of_series), data_series ))))
        reduced = ft.reduce(lambda a,b: (a+b),sum_list)
        reduced /= num_of_items
        series_standard_deviation = reduced**(1/2)
    ...
    def question0(self):
        ...
        '''
        #### FILL IN CODE #### START
        aabbaa = [oil_consumption_per_capita_norm, electricity_consumption_per_capita_norm, gas_consumption_per_capita_norm]
        energy_consumption_average = self.compute_average_across_series(aabbaa)
        energy_consumption_std = self.compute_standard_deviation_across_series(aabbaa, energy_consumption_average)
        #### FILL IN CODE #### END
    ...
    def question1(self):
        ...
        kid_fatality_per_woman = list(map(lambda a,b: ((a*b)/1000), fertility_rate, infant_mortality_rate ))
        # for i in range(len(fertility_rate)):
        #     kid_fatality_per_woman.append((fertility_rate[i]*infant_mortality_rate[i])/1000)
        net_fertility_rate = list(map( lambda a,b: (a-b), fertility_rate, kid_fatality_per_woman ))
        # for j in range(len(fertility_rate)):
        #     net_fertility_rate.append(fertility_rate[j] - kid_fatality_per_woman[j])
        ratio_survival_over_fatality_per_woman = list(map( lambda a,b: (a/b), net_fertility_rate, kid_fatality_per_woman ))
        # for k in range(len(net_fertility_rate)):
        #     ratio_survival_over_fatality_per_woman.append(net_fertility_rate[k]/kid_fatality_per_woman[k])
        #### FILL IN CODE #### END
    ...
    def question2(self):
        ...
        '''
        #### FILL IN CODE #### START
        net_birth_rate = list(map(lambda a,b: (a-b), birth_rate, death_rate))
        #### FILL IN CODE #### END
        ...
        #### FILL IN CODE #### START
        for j in range(len(countries_raw)):
            if(gdppc_raw[j] > gdp_per_capita_threshold):
                high_net_birth_rate_countries.append(countries_raw[j])
            else:
                low_net_birth_rate_countries.append(countries_raw[j])
        #### FILL IN CODE #### END
        #print(high_net_birth_rate_countries)
        #print(low_net_birth_rate_countries)
        '''
        ...
        '''
        #### FILL IN CODE #### START
        high_numbers = list(filter(lambda a: (a > gdp_per_capita_threshold), gdppc_raw))
        low_numbers = list(filter(lambda a: (a <= gdp_per_capita_threshold), gdppc_raw))
        high_net_birthrate_gdppc_average = self.compute_average_of_series(high_numbers)
        high_net_birthrate_gdppc_std = self.compute_standard_deviation_of_series(high_numbers, high_net_birthrate_gdppc_average)
        low_net_birthrate_gdppc_average = self.compute_average_of_series(low_numbers)
        low_net_birthrate_gdppc_std = self.compute_standard_deviation_of_series(low_numbers, low_net_birthrate_gdppc_average)
        #### FILL IN CODE #### END
    ...
    def question3(self,energy_consumption_average_IN,ratio_kids_survive_over_kids_fatality_IN):
        ...
        '''
        #### FILL IN CODE #### START
        t1 = list(map(lambda a: 0.01+(a*((0.12 - 0.01)/9)), range(10)))
        #print(t1)
        # val = 0.01
        # for i in range(10):
        #     t1.append(val)
        ...
        '''
        #### FILL IN CODE #### START
        t2 = list(map(lambda a: 30+(a*((270-30)/9)), range(10)))
        #print(t2)
        # val2 = 30
        # for i in range(10):
        #     t2.append(val2)
